    ...
    <Link to={`/artwork/${artwork.id}`} className="group block">
      <div className="relative overflow-hidden rounded-2xl bg-white shadow-sm hover:shadow-xl transition-all duration-500">
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          ...
          >
            <Heart className={`w-5 h-5 ${inWishlist ? 'fill-amber-700 text-amber-700' : 'text-neutral-700'}`} />
          </button>
          <Link
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-2">
            <div className="flex-1">
              <h3 className="font-serif text-neutral-900 mb-1">{artwork.title}</h3>
              <p className="text-sm text-neutral-500">{artwork.category}</p>
            </div>
            <div className="text-right">
              <div className="font-serif text-amber-700">${artwork.price.toLocaleString()}</div>
            </div>
          </div>
          <div className="text-xs text-neutral-400 mb-4">{artwork.dimensions}</div>
